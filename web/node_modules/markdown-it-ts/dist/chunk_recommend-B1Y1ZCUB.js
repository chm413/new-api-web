import { E as Token, _ as fromCodePoint, a as setChunkDiagnostics, c as setUnboundedDiagnostics, d as finalizeKnownGlobalMarkdownState, f as getKnownGlobalMarkdownState, g as countLines, h as runWithKnownGlobalMarkdownState, i as getParseDiagnostics, l as detectGlobalMarkdownState, m as resetKnownGlobalMarkdownState, n as beginParseDiagnostics, p as markKnownGlobalMarkdownState, x as isValidEntityCode } from "./strategy_diagnostics-CUrGFeN1.js";
...
function parseStockFast(src) {
	...
	while (pos < src.length) {
		...
		if (plainInlineCacheMode === PLAIN_CACHE_DISABLED) paragraphIsPlain = isPlainInlineText(paragraph);
		else if (plainInlineCacheMode === PLAIN_CACHE_ENABLED && paragraph === lastPlainInline) paragraphIsPlain = lastPlainInlineOk;
		else {
...
function renderFence(src, pos, end, cache) {
	...
	while (scan < src.length) {
		...
		if (startsWithFence(src, scan, closeEnd) && isBlankLine$2(src, scan + 3, closeEnd)) {
			const content = src.slice(contentStart, contentEnd);
			let open;
			if (lang === cache.lang) open = cache.open;
			else {
...
function renderStockFast(src) {
	...
	while (pos < src.length) {
		...
		if (ch === 45) {
			...
			let listHtml;
			if (listSource === lastListSource) listHtml = lastListHtml;
			else {
				let itemPos = pos;
				listHtml = "<ul>\n";
		...
		let paragraphHtml;
		if (paragraph === lastParagraph) paragraphHtml = lastParagraphHtml;
		else {
...
//#endregion
export { parseStockFast as _, parseAsyncIterable as a, parseIterableToSink as c, chunkedParse as d, splitIntoChunkRanges as f, unescapeAll as g, escapeHtml as h, getAutoUnboundedDecision as i, parseStringUnbounded as l, renderStockFast as m, recommendStreamChunkStrategy as n, parseAsyncIterableToSink as o, splitIntoChunks as p, UnboundedBuffer as r, parseIterable as s, recommendFullChunkStrategy as t, shouldAutoUseUnbounded as u, parseStockFastAstJson as v };