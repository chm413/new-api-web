...
//#endregion
//#region ../../node_modules/.pnpm/markdown-it-ts@1.0.4/node_modules/markdown-it-ts/dist/strategy_diagnostics-CUrGFeN1.js
...
//#endregion
//#region ../../node_modules/.pnpm/markdown-it-ts@1.0.4/node_modules/markdown-it-ts/dist/parse-yL3UtJgx.js
...
//#endregion
//#region ../../node_modules/.pnpm/markdown-it-ts@1.0.4/node_modules/markdown-it-ts/dist/chunk_recommend-B1Y1ZCUB.js
...
function pushTightBulletListItem(tokens, content, line) {
	...
	const inline$1 = blockToken("inline", "", 0, 3);
	inline$1.map = [line, line + 1];
	inline$1.content = content;
	const child = new Token("text", "", 0);
	child.content = content;
	inline$1.children = [child];
...
function parseStockFast(src) {
	...
	while (pos < src.length) {
		...
		if (plainInlineCacheMode === PLAIN_CACHE_DISABLED) paragraphIsPlain = isPlainInlineText(paragraph$1);
		else if (plainInlineCacheMode === PLAIN_CACHE_ENABLED && paragraph$1 === lastPlainInline) paragraphIsPlain = lastPlainInlineOk;
		else {
...
function renderFence$1(src, pos, end, cache) {
...
function renderStockFast(src) {
	...
	while (pos < src.length) {
		...
		if (ch === 96) {
			const fence$1 = renderFence$1(src, pos, end, fenceOpenCache);
			if (!fence$1) return null;
		...
		let paragraphHtml;
		if (paragraph$1 === lastParagraph) paragraphHtml = lastParagraphHtml;
		else {
...
//#endregion
//#region ../../node_modules/.pnpm/markdown-it-ts@1.0.4/node_modules/markdown-it-ts/dist/index.js
...
function inlineTokenMayNeedLinkify(token, linkify$2) {
	...
	for (let index = children.length - 1; index >= 0; index--) {
		...
		if (htmlLinkLevel > 0) continue;
		if (currentToken?.type === "text" && linkify$2.pretest(String(currentToken.content ?? ""))) return true;
	}
	return false;
}
...
function isCjkClosingPunctuation(ch, next) {
	return !!ch && !!next && /^\p{Script=Han}$/u.test(next) && CJK_CLOSING_PUNCTUATION_RE.test(ch);
}
