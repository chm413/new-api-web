import { BasePlugin, ChartEvent, registerChartPlugin, isValid } from "@visactor/vchart";

const EXTENSION_MARK_SYNC_STATE_PLUGIN_TYPE = "ExtensionMarkSyncStatePlugin";

export class ExtensionMarkSyncStatePlugin extends BasePlugin {
    constructor() {
    ...
    onAfterInitChart(service, chartSpec) {
        ...
        event.on(ChartEvent.afterRender, handler), this._subscribedEvent = event, this._unsubscribeAfterRender = () => {
            event.off(ChartEvent.afterRender, handler), this._subscribedEvent = void 0;
        };
    }
    release() {
        var _a;
    ...
    _syncSeriesStates(series, extensionMarkSpecs) {
        ...
        if (series.getActiveMarks().forEach((mark => {
            mark.getGraphics().forEach((g => {
                ...
                isValid(key) && mainGraphicByKey.set(String(key), g);
            }));
        })), 0 === mainGraphicByKey.size) return;
        const namePrefix = `${series.type}_${series.id}_extensionMark`;
        extensionMarkSpecs.forEach(((spec, i) => {
            const markName = isValid(spec.name) ? `${spec.name}` : `${namePrefix}_${i}`, extMark = series.getMarkInName(markName);
...
ExtensionMarkSyncStatePlugin.pluginType = "chart", ExtensionMarkSyncStatePlugin.type = "ExtensionMarkSyncStatePlugin";
