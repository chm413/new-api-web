(function (global, factory) {
    ...
    const findPluginByParser = (parserName, options) => {
        ...
        const searchPlugins = jsdocIndex === -1 ? plugins : plugins.slice(0, jsdocIndex);
    ...
    function getMergedParser(originalParser, parserName) {
    ...
    const parsers = {
        get babel() {
            return getMergedParser(parserBabel__default["default"].parsers.babel, "babel");
        },
        get "babel-flow"() {
            return getMergedParser(parserBabel__default["default"].parsers["babel-flow"], "babel-flow");
        },
        get "babel-ts"() {
            return getMergedParser(parserBabel__default["default"].parsers["babel-ts"], "babel-ts");
        },
        get flow() {
            return getMergedParser(parserFlow__default["default"].parsers.flow, "flow");
        },
        get typescript() {
            return getMergedParser(parserTypescript__default["default"].parsers.typescript, "typescript");
        },
        get "jsdoc-parser"() {
            return getMergedParser(parserBabel__default["default"].parsers["babel-ts"], "babel-ts");
        },
    ...
    function mergeParsers(originalParser, parserName) {
        const activePreprocesses = new WeakSet();
        const activeParses = new WeakSet();
        const innerParse = getParser(originalParser.parse, parserName);
        const jsDocParse = async (text, parsersOrOptions, maybeOptions) => {
