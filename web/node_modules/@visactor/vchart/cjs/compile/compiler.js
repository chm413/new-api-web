...
const event_1 = require("./../constant/event"), interface_1 = require("./interface"), env_1 = require("../util/env"), type_1 = require("../util/type"), vutils_1 = require("@visactor/vutils"), vrender_bridge_1 = require("../vrender-bridge"), factory_1 = require("../core/factory"), mark_1 = require("../util/mark"), util_1 = require("./util"), debug_1 = require("../util/debug"), interface_2 = require("../animation/interface"), animate_1 = require("../constant/animate"), stage_app_1 = require("./stage-app"), POPTIP_FOR_TEXT_PLUGIN_NAME = "poptipForText";
...
class Compiler {
    ...
    constructor(container, option) {
        this._count = 0, this._rootMarks = [], this._isExternalStage = !1, this._viewListeners = new Map, 
        this._windowListeners = new Map, this._canvasListeners = new Map, this.isInited = !1, 
        this._released = !1, this._compileChart = null, this.handleLayoutEnd = () => {
            var _a, _b;
    ...
    initView() {
        ...
        const {autoRefreshDpr: autoRefreshDpr, dpr: dpr, mode: mode, modeParams: modeParams, gestureConfig: gestureConfig, interactive: interactive, clickInterval: clickInterval, autoPreventDefault: autoPreventDefault, background: background} = this._option, appEnvParams = (0, 
        stage_app_1.getVRenderAppEnvParams)(mode, modeParams);
        vrender_bridge_1.vglobal.setEnv((0, util_1.toRenderMode)(mode), null != appEnvParams ? appEnvParams : {});
        const externalStage = this._option.stage;
        if (this._isExternalStage = !!externalStage, this._releaseVRenderAppRef = void 0, 
        this._stage = externalStage, !this._stage) {
            const resolvedApp = (0, stage_app_1.resolveVRenderApp)({
            ...
            try {
                this._stage = (0, stage_app_1.createStageFromApp)(resolvedApp.app, Object.assign({
                ...
                throw null === (_e = this._releaseVRenderAppRef) || void 0 === _e || _e.call(this), 
                this._releaseVRenderAppRef = void 0, error;
            }
        }
        ...
        const group = (0, vrender_bridge_1.createGroup)({
            x: 0,
            y: 0,
            width: this._width,
            height: this._height
    ...
    _isGeoRegionRoamDragEnabled() {
        var _a, _b;
        const chartSpec = null === (_b = null === (_a = this._compileChart) || void 0 === _a ? void 0 : _a.getSpec) || void 0 === _b ? void 0 : _b.call(_a), regions = null == chartSpec ? void 0 : chartSpec.region;
        return !!(0, vutils_1.isArray)(regions) && regions.some((region => {
            var _a;
    ...
    _shouldDisableCanvasTouchAction() {
        if (!(0, env_1.isTrueBrowser)(this._option.mode)) return !1;
        return !1 === ((0, vutils_1.isValid)(this._option.supportsTouchEvents) ? this._option.supportsTouchEvents : vrender_bridge_1.vglobal.supportsTouchEvents) && this._isGeoRegionRoamDragEnabled();
    }
    _setCanvasStyle() {
    ...
    compile(ctx, option) {
        ...
        this._cachedMarks = null), chart.compile(), this._setCanvasStyle(), chart.afterCompile());
    }
    clearNextRender() {
        return !!this._nextRafId && (vrender_bridge_1.vglobal.getSpecifiedCancelAnimationFrame(10)(this._nextRafId), 
    ...
    renderNextTick(morphConfig) {
        this._released || (this._nextRafId && this.clearNextRender(), this._nextRafId = vrender_bridge_1.vglobal.getSpecifiedRequestAnimationFrame(10)((() => {
    ...
    renderMarks() {
        ...
        const needsLayout = this._layoutState === interface_1.LayoutState.before;
        (hasCommitedMark || needsLayout) && ((0, debug_1.log)(`--- start of renderMarks(${this._count}) ---`), 
        this.clearProgressive(), hasCommitedMark && this._rootMarks.forEach((mark => {
            mark.render();
        })), needsLayout && (this._layoutState = interface_1.LayoutState.layouting, null === (_a = this._compileChart) || void 0 === _a || _a.onLayout(), 
        this._layoutState = interface_1.LayoutState.reevaluate, hasCommitedMark = this._hasCommitedMark(), 
        hasCommitedMark && this._rootMarks.forEach((mark => {
            mark.render();
        })), this.handleLayoutEnd()), hasCommitedMark && (this.findProgressiveMarks(), this.updateStateAnimation()), 
        this._doRender(!0), hasCommitedMark && this.doPreProgressive(), (0, debug_1.log)(`--- start of renderMarks(${this._count}) ---`), 
        this._count++);
    }
    ...
    release() {
        const stage = this._stage, rootGroup = this._rootGroup, shouldReleaseStage = !!stage && !this._isExternalStage, releaseVRenderAppRef = this._releaseVRenderAppRef;
        this.clearNextRender(), this.releaseEvent(), this.releaseGrammar(!0), stage && (shouldReleaseStage ? stage.release() : rootGroup && (stage.defaultLayer.removeChild(rootGroup, !0), 
        rootGroup.release())), null == releaseVRenderAppRef || releaseVRenderAppRef(), this._stage = null, 
        this._rootGroup = null, this._isExternalStage = !1, this._releaseVRenderAppRef = void 0, 
        this._option = this._container = null, this.isInited = !1, this._compileChart = null, 
        this._released = !0;
    }
    ...
    _combineIncrementalLayers() {
        this._stage && (0, vrender_bridge_1.waitForAllSubLayers)(this._stage).then((() => {
    ...
    doPreProgressive() {
        if (this._progressiveMarks && this._progressiveMarks.some((mark => mark.isDoingProgressive()))) {
            const raf = vrender_bridge_1.vglobal.getSpecifiedRequestAnimationFrame(10);
    ...
    clearProgressive() {
        if (this._progressiveRafId) {
            vrender_bridge_1.vglobal.getSpecifiedCancelAnimationFrame(10)(this._progressiveRafId);
        }
