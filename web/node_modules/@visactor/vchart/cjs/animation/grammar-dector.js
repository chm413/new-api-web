...
Object.defineProperty(exports, "__esModule", {
    value: !0
}), exports.GrammarDetector = void 0;

const animation_planner_1 = require("./animation-planner"), enum_1 = require("../mark/interface/enum"), vertical_bar_split_1 = require("./strategy/vertical-bar-split"), horizontal_bar_split_1 = require("./strategy/horizontal-bar-split");

class GrammarDetector {
    constructor(mark) {
        this.splitStrategies = [], this.mark = mark, "rect" === mark.type && (this.registerStrategy(new vertical_bar_split_1.VerticalBarSplitStrategy), 
        this.registerStrategy(new horizontal_bar_split_1.HorizontalBarSplitStrategy));
    }
    ...
    detect(graphics, graphicMap) {
        ...
        return graphics.forEach((g => {
            ...
            switch (g.context.animationState) {
              ...
              default:
                g.context.diffState === enum_1.DiffState.exit ? exitGraphics.push(g) : g.context.diffState === enum_1.DiffState.enter ? enterGraphics.push(g) : updateGraphics.push(g);
            }
        })), graphicMap.size > graphics.length && graphicMap.forEach(((g, key) => {
            g.context.diffState !== enum_1.DiffState.exit || g.isExiting || exitGraphics.push(g);
        })), {
    ...
    createPlanners(result, animationConfig) {
        const planners = [];
        if (result.hasExit && animationConfig.exit && planners.push(new animation_planner_1.AnimationPlanner("exit", result.exitGraphics, animationConfig.exit)), 
        result.hasUpdate && animationConfig.update) {
            ...
            })), regularUpdateGraphics.length > 0 && planners.push(new animation_planner_1.AnimationPlanner("update", regularUpdateGraphics, animationConfig.update));
            ...
            steps.forEach((step => {
                ...
                if (graphicsWithSteps && graphicsWithSteps.length > 0) {
                    ...
                    planners.push(new animation_planner_1.AnimationPlanner("update", graphicsForStep.map((item => item.graphic)), animationConfig.update, (graphics => {
        ...
        return result.hasEnter && animationConfig.enter && planners.push(new animation_planner_1.AnimationPlanner("enter", result.enterGraphics, animationConfig.enter)), 
        result.hasAppear && animationConfig.appear && planners.push(new animation_planner_1.AnimationPlanner("appear", result.appearGraphics, animationConfig.appear)), 
        planners;
    }
}
