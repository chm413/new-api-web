...
export class BarSeries extends CartesianSeries {
    ...
    _initStackBarMarkStyle() {
        ...
        if (!this._spec.stackCornerRadius) {
            ...
            return void (!markConfig.clip && isNil(markConfig.clipPath) || this._barMark.setMarkConfig({
                clip: !1,
                clipPath: []
            }));
        }
                ...
                return this._forEachStackGroup((node => {
                    ...
                    node.values.forEach((datum => {
                        ...
                        if (min = Math.min(min, start, end), max = Math.max(max, start, end), usePreCalculatedRect) {
                            const rectStart = datum[isVertical ? RECT_Y : RECT_X], rectEnd = datum[isVertical ? RECT_Y1 : RECT_X1];
                            rectMin = Math.min(rectMin, rectStart, rectEnd), rectMax = Math.max(rectMax, rectStart, rectEnd);
                        }
                        isValid(startPercent) && isValid(endPercent) && (hasPercent = !0, minPercent = Math.min(minPercent, startPercent, endPercent), 
    ...
    _getBarWidth(axisHelper, scaleDepth) {
        var _a;
        const depthFromSpec = this._groups ? this._groups.fields.length : 1, depth = isNil(scaleDepth) ? depthFromSpec : Math.min(depthFromSpec, scaleDepth), hasBarWidth = isValid(this._spec.barWidth) && depth === depthFromSpec, axisBandWidth = hasBarWidth && "number" == typeof this._spec.barWidth && "string" != typeof this._spec.barMinWidth && "string" != typeof this._spec.barMaxWidth || null === (_a = axisHelper.getBandwidth) || void 0 === _a ? void 0 : _a.call(axisHelper, depth - 1), bandWidth = null != axisBandWidth ? axisBandWidth : 6, hasBarMinWidth = isValid(this._spec.barMinWidth), hasBarMaxWidth = isValid(this._spec.barMaxWidth);
        let width = !axisHelper.isContinuous || isNil(axisBandWidth) || hasBarWidth ? bandWidth : .5 * bandWidth;
