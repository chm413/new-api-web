...
export class Compiler {
    ...
    initView() {
        ...
        const {autoRefreshDpr: autoRefreshDpr, dpr: dpr, mode: mode, modeParams: modeParams, gestureConfig: gestureConfig, interactive: interactive, clickInterval: clickInterval, autoPreventDefault: autoPreventDefault, background: background} = this._option, appEnvParams = getVRenderAppEnvParams(mode, modeParams);
        vglobal.setEnv(toRenderMode(mode), null != appEnvParams ? appEnvParams : {});
    ...
    _isGeoRegionRoamDragEnabled() {
        ...
        return !!isArray(regions) && regions.some((region => {
            var _a;
    ...
    _shouldDisableCanvasTouchAction() {
        ...
        return !1 === (isValid(this._option.supportsTouchEvents) ? this._option.supportsTouchEvents : vglobal.supportsTouchEvents) && this._isGeoRegionRoamDragEnabled();
    }
    _setCanvasStyle() {
    ...
    renderMarks() {
        ...
        (hasCommitedMark || needsLayout) && (log(`--- start of renderMarks(${this._count}) ---`), 
        this.clearProgressive(), hasCommitedMark && this._rootMarks.forEach((mark => {
            mark.render();
        })), needsLayout && (this._layoutState = LayoutState.layouting, null === (_a = this._compileChart) || void 0 === _a || _a.onLayout(), 
        this._layoutState = LayoutState.reevaluate, hasCommitedMark = this._hasCommitedMark(), 
        ...
        this._doRender(!0), hasCommitedMark && this.doPreProgressive(), log(`--- start of renderMarks(${this._count}) ---`), 
        this._count++);
    }
