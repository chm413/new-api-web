...
class AxisBase extends animate_component_1.AnimateComponent {
    ...
    getBoundsWithoutRender(attributes) {
        ...
        const offscreenGroup = graphic_creator_1.graphicCreator.group({
            x: this.attribute.x,
            y: this.attribute.y
        });
    ...
    _renderInner(container) {
        const {title: title, label: label, tick: tick, line: line, items: items} = this.attribute, axisContainer = graphic_creator_1.graphicCreator.group({
            x: 0,
            y: 0,
            zIndex: 1,
            pickable: !1
        ...
        tick && tick.visible && this.renderTicks(axisContainer), label && label.visible)) {
            const labelGroup = graphic_creator_1.graphicCreator.group({
                x: 0,
                y: 0,
                pickable: !1
            });
    ...
    renderTicks(container) {
        const tickLineItems = this.getTickLineItems(), tickLineGroup = graphic_creator_1.graphicCreator.group({
            x: 0,
            y: 0,
            pickable: !1
        });
        ...
        container.add(tickLineGroup), tickLineItems.forEach(((item, index) => {
            var _a;
            const line = graphic_creator_1.graphicCreator.line(Object.assign({}, this._getTickLineAttribute("tick", item, index, tickLineItems)));
        ...
        if (subTick && subTick.visible) {
            ...
            subTickLineItems.length && subTickLineItems.forEach(((item, index) => {
                const line = graphic_creator_1.graphicCreator.line(Object.assign({}, this._getTickLineAttribute("subTick", item, index, tickLineItems)));
    ...
    renderLabels(container, items, layer) {
        ...
        const data = this._transformItems(items), labelGroup = graphic_creator_1.graphicCreator.group({
            x: 0,
            y: 0,
            pickable: !1
        });
