...
const vutils_1 = require("@visactor/vutils"), layout_1 = require("../core/contributions/textMeasure/layout"), interface_1 = require("../interface"), application_1 = require("../application"), text_1 = require("./text"), utils_1 = require("../common/utils"), WRAP_TEXT_UPDATE_TAG_KEY = [ "heightLimit", "lineClamp" ];
        ...
        lineClamp && (lineCountLimit = Math.min(lineCountLimit, lineClamp)), "number" == typeof maxLineWidth && maxLineWidth !== 1 / 0) {
            if (maxLineWidth > 0) for (let i = 0; i < lines.length; i++) {
                ...
                if (i === lineCountLimit - 1) {
                    const clip = layoutObj.textMeasure.clipTextWithSuffix(str, layoutObj.textOptions, maxLineWidth, ellipsis, !1, suffixPosition), metrics = layoutObj.textMeasure.measureTextPixelADscentAndWidth(clip.str, layoutObj.textOptions, interface_1.MeasureModeEnum.actualBounding);
                    linesLayout.push({
                        str: clip.str,
                        width: clip.width,
                ...
                const metrics = layoutObj.textMeasure.measureTextPixelADscentAndWidth(clip.str, layoutObj.textOptions, interface_1.MeasureModeEnum.actualBounding);
                if (linesLayout.push({
                    str: clip.str,
                    width: clip.width,
                    ascent: metrics.ascent,
            ...
            for (let i = 0, len = lines.length; i < len; i++) {
                if (i === lineCountLimit - 1) {
                    const clip = layoutObj.textMeasure.clipTextWithSuffix(lines[i], layoutObj.textOptions, maxLineWidth, ellipsis, !1, suffixPosition), metrics = layoutObj.textMeasure.measureTextPixelADscentAndWidth(clip.str, layoutObj.textOptions, interface_1.MeasureModeEnum.actualBounding);
                    linesLayout.push({
                        str: clip.str,
                        width: clip.width,
                        ascent: metrics.ascent,
                ...
                const metrics = layoutObj.textMeasure.measureTextPixelADscentAndWidth(text, layoutObj.textOptions, interface_1.MeasureModeEnum.actualBounding);
