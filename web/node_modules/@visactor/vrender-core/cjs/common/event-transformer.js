...
function cloneTouchList(touchList) {
    return touchList && 0 !== touchList.length ? Array.from(touchList, (touch => Object.assign({}, touch))) : [];
}

function cloneTouchEvent(event) {
    var _a;
    const transformedEvent = Object.create(null !== (_a = Object.getPrototypeOf(event)) && void 0 !== _a ? _a : Object.prototype);
    return Object.defineProperties(transformedEvent, {
        type: {
            value: event.type,
            configurable: !0
        },
        touches: {
            value: cloneTouchList(event.touches),
            configurable: !0
        },
        changedTouches: {
            value: cloneTouchList(event.changedTouches),
            configurable: !0
        }
    }), transformedEvent;
}

function cloneNativeEvent(event) {
    return event instanceof TouchEvent ? cloneTouchEvent(event) : new event.constructor(event.type, event);
}

function getSafeEventProperty(event, key) {
    try {
        return event[key];
    } catch (error) {
        return;
...
function createEventTransformer(containerElement, getMatrix, getRect, transformPoint) {
    return event => {
        ...
        const containerRect = getRect(), transformedEvent = cloneNativeEvent(event), target = getSafeEventProperty(event, "target"), currentTarget = getSafeEventProperty(event, "currentTarget");
        if (Object.defineProperties(transformedEvent, {
            ...
            const transformedTouchEvent = transformedEvent;
