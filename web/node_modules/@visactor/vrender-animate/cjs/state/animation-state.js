...
class AnimationStateManager {
    ...
    takeOverUpdateAttrs(nextState, currentStates, shouldStopState) {
        if (nextState.state !== exports.AnimationStates.UPDATE || !currentStates.length) return;
        ...
        if (nextKeys.length) for (let i = 0; i < currentStates.length; i++) {
            ...
            currentState.state !== exports.AnimationStates.UPDATE || currentState === nextState || this.hasStateInfo(shouldStopState, currentState) || (currentState.executor.preventAttrs(nextKeys) || (currentState.executor.stop(null, !1), 
            shouldStopState.push(currentState)));
        }
    }
    applyState(nextState, animationConfig, callback) {
        var _a;
        const registry = animation_states_registry_1.AnimationTransitionRegistry.getInstance(), currentStateList = null !== (_a = this.stateList) && void 0 !== _a ? _a : [], shouldStopState = [], shouldApplyState = [];
